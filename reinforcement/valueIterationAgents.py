...
class ValueIterationAgent(ValueEstimationAgent):
          ...
          for state in self.mdp.getStates():
            ...
            for action in possibleActions:
              ...
              for transition in transitionStatesAndProbs:
                valueState += transition[1] * (self.mdp.getReward(state, action, transition[0]) + self.discount * currValue[transition[0]])
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for StateAndProb in TransStatesAndProbs:
          next_state = StateAndProb[0]
          reward = self.mdp.getReward(state, action, next_state)
          Qvalue += StateAndProb[1]*(reward+self.discount*self.getValue(next_state))
        return Qvalue
